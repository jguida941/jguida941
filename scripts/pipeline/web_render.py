...
.chip { display: inline-flex; align-items: center; gap: 6px; font-size: var(--type-chip); font-weight: var(--type-chip-weight);
  padding: 6px 11px; border-radius: 999px; color: var(--ink); }
.chip.ok { background: color-mix(in srgb, var(--status-success) 16%, transparent); color: color-mix(in srgb, var(--status-success) 92%, var(--ink-strong)); }
.chip.warn { background: color-mix(in srgb, var(--status-warning) 16%, transparent); color: color-mix(in srgb, var(--status-warning) 92%, var(--ink-strong)); }
.chip.bad { background: color-mix(in srgb, var(--status-danger) 16%, transparent); color: color-mix(in srgb, var(--status-danger) 92%, var(--ink-strong)); }
...
@media (max-width: 760px) {
  .bento, .tiles, .langlegend, .lanes { grid-template-columns: 1fr; }
  .hero .kpi { gap: 18px; }
  .hero .stats { gap: 18px; }
}
...
def _rhythm() -> str:
  ...
  <section class="panel" id="rhythm-panel" hidden>
    ...
    <hr class="hairline">
    <div class="heat"><div class="heat-days" id="heat-days"></div><div class="heat-grid" id="heat-grid"></div><div class="heat-hours" id="heat-hours"></div></div>
...
def _script() -> str:
  ...
  const safeUrl = (u) => /^https?:\x2F\x2F/i.test(String(u||"")) ? esc(u) : "#";
  ...
  function hydrate(d) {
    ...
    document.getElementById("pipeline").innerHTML = PIPE.map(([nm,k]) => {
      const s = q[k] || "unknown";
      return `<span class="chip ${cls(s)}">${nm} · ${s==="ok"?"OK":esc(s[0].toUpperCase()+s.slice(1))}</span>`;
    }).join("");
