...
class PageLayoutContract(unittest.TestCase):
    ...
    def test_conform_emits_passing_page_layout_rows(self):
        from scripts.quality.design_invariants import conform
        for name in _active():
            rows = [r for r in conform(name) if r["aspect"] == "page-layout"]
            self.assertEqual(len(rows), 3, f"{name}: 3 page-layout invariants (column/hierarchy/density)")
            for r in rows:
                self.assertEqual(r["status"], "pass", f"{name}: {r['invariant_id']} must pass")
