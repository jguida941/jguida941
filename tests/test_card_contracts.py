...
def _render(card, out):
    ...
    if card == "metrics_general":
        from scripts.generate_metrics_general import generate
        return generate(username=m["dashboard_data"]["username"], snapshot=m["snapshot"],
                        ...
                        output_path=out)
    if card == "badges":
        from scripts.generate_badges import generate
    ...
    if card == "scorecard":
        from scripts.generate_builder_scorecard import generate
        return generate(m["scorecard"], output_path=out, tiles=m["scorecard_cards"])
    if card == "engineering":
        from scripts.generate_engineering_cadence import generate
        return generate(m["engineering"], output_path=out)
    if card == "focus":
        from scripts.generate_focus_board import generate
        return generate(m["focus"], output_path=out)
    if card == "currently_working":
        from scripts.generate_currently_working import generate
    ...
    if card == "snapshot":
        from scripts.generate_snapshot_panel import generate
        return generate(m["snapshot_rows"], m["data_quality"], data_scope=m["data_scope"], output_path=out)
    if card == "lang":
        from scripts.generate_language_chart import generate
        return generate(c.language_bytes, output_path=out)
    if card == "heatmap":
        from scripts.generate_activity_heatmap import generate
        return generate(c.events, output_path=out)
    if card == "contribution":
        from scripts.generate_contribution_panel import generate
        return generate(c.calendar, output_path=out)
    if card == "spotlight":
        from scripts.generate_repo_spotlight import generate
