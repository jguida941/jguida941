...
### Single-worktree hard block (operator-ratified 2026-07-22)

The repository operates from one authoritative worktree and one durable development branch:
`w3-correction`. Creating another Git worktree is forbidden. No conductor or worker may run
`git worktree add`, create a detached build/review directory, or treat files outside the
authoritative checkout as live state. Workers receive closed path scopes inside the authoritative
checkout and must not overlap writes. Any pre-existing worktree is recovery debt: before removal,
`/root` records its HEAD, dirty-path roster, and deterministic patch hash; its bytes are then
integrated into `w3-correction` or preserved with a typed `SUPERSEDED` / `REJECTED` disposition.
After the current grandfathered builder exits, no active exception remains. A new worktree is a
hard process failure and blocks dispatch.
...
(b) Build lanes: after a slice's design carries `DESIGN-VERDICT: APPROVE` or lawful executable-
evidence convergence and its admission
