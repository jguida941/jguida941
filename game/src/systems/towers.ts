...
import {
  getClassBuildCostMultiplier,
  getClassDamageMultiplier,
  getClassFireRateMultiplier,
  getClassRangeMultiplier,
} from "./classes.js";
...
import type { EnemyState, GameState, PathModel, TowerKind, TowerState, Vec2 } from "../core/types.js";

function towerPositionFromSlot(state: GameState, tower: TowerState): Vec2 {
...
function buildCost(state: GameState, kind: TowerKind): number {
...
export function tryPlaceTower(state: GameState, slotId: number, kind: TowerKind): boolean {
  if (state.phase !== "prep") {
...
export function updateTowers(state: GameState, path: PathModel, dt: number): void {
