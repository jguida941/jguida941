...
function addPacket(packets, kind, count, cadence) {
    if (count <= 0) {
        return;
    }
    packets.push({ kind, count, cadence });
}
export function buildWavePackets(stage, wave) {
    const packets = [];
    const pressure = stage * 1.45 + wave;
    addPacket(packets, "grunt", 6 + Math.floor(pressure * 2.2), Math.max(0.27, 0.6 - pressure * 0.02));
    if (wave >= 2) {
        addPacket(packets, "runner", 2 + Math.floor(pressure * 1.5), Math.max(0.22, 0.48 - pressure * 0.015));
    }
    if (wave >= 3) {
        addPacket(packets, "tank", 1 + Math.floor((stage + wave) / 2), Math.max(0.55, 0.88 - pressure * 0.02));
        addPacket(packets, "splitter", 1 + Math.floor((stage + wave) / 3), Math.max(0.48, 0.78 - pressure * 0.016));
    }
    if (stage >= 2 && wave >= 3) {
        addPacket(packets, "bomber", 1 + Math.floor((stage + wave) / 3), Math.max(0.42, 0.66 - pressure * 0.012));
    }
    if (stage >= 3 && wave >= 4) {
        addPacket(packets, "shaman", 1 + Math.floor((stage + wave) / 4), Math.max(0.68, 0.96 - pressure * 0.015));
    }
    if (wave === 5) {
...
function packetsToPending(packets) {
...
export function updateWaveSpawning(state, dt) {
...
export function resolveWaveState(state) {
    if (state.phase !== "combat") {
        return;
    }
