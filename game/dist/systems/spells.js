...
function ensureSpellState(state, kind) {
    return state.spells[kind];
}
export function recomputeSpellCapacities(state) {
...
function castMeteor(state, path) {
...
function castHealingAura(state) {
    const spellPower = getClassSpellPowerMultiplier(state);
...
function spendSpellCharge(state, spell) {
...
export function tryCastSpell(state, path, spellKind) {
...
function advanceSingleSpellCooldown(spell, dt, rechargeMultiplier) {
    if (spell.charges >= spell.maxCharges) {
        spell.cooldownRemaining = 0;
        return;
    }
...
export function updateSpells(state, dt) {
...
export function getSpellChargeLabel(state, spellKind) {
...
export function getLeadEnemyPosition(state, path) {
    const lead = pickLeadEnemy(state);
    if (!lead) {
        return null;
    }
