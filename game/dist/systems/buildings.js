...
function buildingCost(state, kind) {
...
export function getBuildingCount(state, kind) {
    let count = 0;
...
export function getSpellLabBonusCharges(state) {
    return getBuildingCount(state, "spell_lab");
}
export function getSpellLabRechargeMultiplier(state) {
    const labs = getBuildingCount(state, "spell_lab");
    return 1 + labs * 0.22;
}
export function getBaseDamageTakenMultiplier(state) {
    const barracks = getBuildingCount(state, "barracks");
    return Math.max(0.5, 1 - barracks * 0.12);
}
export function tryPlaceBuilding(state, slotId, kind) {
    if (state.phase !== "prep") {
...
function tickGoldMine(state, building, dt) {
    building.timer += dt;
...
function tickBarracks(state, building, dt) {
    building.timer += dt;
