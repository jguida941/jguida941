...
function findClickedSlot(point, slots) {
...
export function bindInput(bindings, enqueue, getTowerSlots) {
    const disposers = [];
    ...
    for (const [heroClass, button] of Object.entries(bindings.classButtons)) {
        const handler = () => enqueue({ type: "select_class", heroClass });
        button.addEventListener("click", handler);
        disposers.push(() => button.removeEventListener("click", handler));
    }
    for (const [towerKind, button] of Object.entries(bindings.towerButtons)) {
        const handler = () => enqueue({ type: "select_tower", tower: towerKind });
        button.addEventListener("click", handler);
        disposers.push(() => button.removeEventListener("click", handler));
    }
    for (const [buildingKind, button] of Object.entries(bindings.buildingButtons)) {
        const handler = () => enqueue({ type: "select_building", building: buildingKind });
        button.addEventListener("click", handler);
        disposers.push(() => button.removeEventListener("click", handler));
    }
    for (const [spellKind, button] of Object.entries(bindings.spellButtons)) {
        const handler = () => enqueue({ type: "cast_spell", spell: spellKind });
        button.addEventListener("click", handler);
        disposers.push(() => button.removeEventListener("click", handler));
    }
    for (const [upgradeKind, button] of Object.entries(bindings.upgradeButtons)) {
