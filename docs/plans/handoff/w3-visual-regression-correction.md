...
does not erase the repository's ratified role separation. Seat changes require explicit operator
routing recorded in `AGENTS.md`.
...
13. **Respect bounded slices.** Reconcile the dirty main branch/ahead-of-origin state without data
    ...
    only reviewed `w3-correction` commits. Integrated CODE and ADVERSARIAL approval closes slice 10
on that development branch; it does not authorize a merge or push to `main`.
...
reviews. Keep every commit on `w3-correction`; final integrated CODE and ADVERSARIAL gates produce
the slice-10 acceptance checkpoint only. Do not merge or push to `main` without a later separate
operator decision. Preserve the dirty tree and never use destructive cleanup for the boundary.
...
0. **DESIGN closure, no renderer mutation.** Read every linked source/seam. Produce the exact 3
   ...
   W5f schemas; source-evidence schema; and the exact stale clauses/tests to retire. That scratch
   design must reach its binding DESIGN state under the review-effort law before slice 1: an
   independent `DESIGN-VERDICT: APPROVE`, or convergence after two REVISE rounds with every
   finding folded and verified at the slice's CODE + ADVERSARIAL patch gates.
...
9. **Fixture/reference readiness and skill.** Make the minimum W5f lane executable against frozen
   fixtures/local snapshots, update and forward-test `skills/design-language-tdd`, and prove it
   rejects a generic reskin, bogus official claim, unapproved reference, unresolved rendered gap,
   and vacuous mutation. No live external crawl and no portfolio/target write occur in this slice.
10. **Integrated review and clean checkpoint.** Regenerate committed pages/conformance claims, run the focused command and
    `PYTHONDONTWRITEBYTECODE=1 python -m unittest discover -s tests -v`, run every named mutation, then
    obtain final Codex CODE and ADVERSARIAL approval, then stop at the clean `w3-correction`
    acceptance checkpoint; no `main` merge/push is authorized.
...
- The scratch design and finished diff carry exactly one current machine-checkable outcome per
  gate: for DESIGN, `DESIGN-VERDICT: APPROVE` or a recorded convergence closure (two REVISE
  rounds, all findings folded, verified at the patch gates); for the patch, `VERDICT: approve`
  and `ADVERSARIAL-VERDICT: APPROVE`.
