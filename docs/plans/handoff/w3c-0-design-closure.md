# W3C-0 — Slice-0 design closure (rev 16, SELF-CONTAINED, 2026-07-16)
...
> document authorizes NO renderer mutation. §16 is the sole status authority: row A is
> operator-confirmed and row H1 is resolved as design-gated process (both 2026-07-16); rows
> B–G and H2 remain PENDING operator ratification and are never lowerable as Mode-D authority.
...
**DEMO B readiness** (fixture/local candidate → approval → freeze → fidelity-confirm → measure →
promotion proof; slice 9). Slice 10 closes the clean reviewed `w3-correction` checkpoint. The
selected Glukhovsky request and portfolio transformation begin only afterward in a fresh admitted
lane; W6 never widens repo-surface-scout's on-disk probe (`scout_fetch_executed: false`).
...
- **T0 LANDED IN WORKTREE (2026-07-15):** `contracts/doc_authority_policy.toml` +
  ...
  `HANDOFF.md` reentry card refreshed; 12 TRACKED lifecycle rows, 3 live ≤ cap 5 after the
  landing reconciliation) → **8/8 green**.
...
### 1.1 Actor build-lane amendment (operator-confirmed §16-A, 2026-07-16; the `AGENTS.md` edit lands in slice 0's commit)
...
And append this correction-branch clause (design-gated process, resolved §16-H1 2026-07-16;
resolves the r3 conflict with the unamended `AGENTS.md:21` full-suite-green SOP):

> During the `w3-correction` branch, slice 0 commits the honest SEED baseline and is itself
> exempt from ratchet-green: it is the first branch commit, so no parent ledger exists to
> compare against. From slice 1 onward the slice SOP's "full suite green" requirement is
> satisfied by: the slice's focused modules green PLUS the correction failure-ratchet green
> (monotonic shrink at (test id, fingerprint) granularity against the pinned baseline digest
> in `contracts/correction_baseline.json`). Full-suite green is the binding requirement again
...
binds: one plan-of-record; lifecycle rows for every TRACKED handoff doc (an untracked draft
enters the bijection when its lane lands file + row together); caps fail closed.

## 2. MF2 — Authority map (complete and lowerable)

### 2.1 Clause catalog for ratified laws (new, slice 1)

Each `R-W3-*` id becomes a resolvable clause in **`contracts/design_sources/ratified.json`**
only when its authority exists: rows grounded in the gate record or in RESOLVED §16 rows land
in slice 1; a clause whose §16 row is PENDING may not be authored until that row resolves
...
**Motion value proposals are exact and complete but PENDING §16-H2** (r3 must-fix 1; source
`docs/design/motion.md` §2 table, already doctrine): `R-W3-COMP-MOTION-1` proposes per profile —
liquid-glass fast/base/slow = 120/200/400ms, easings ease-in-out/ease-out/ease-in ([derived],
proposed owner choice); carbon = 70/110/240ms with the three productive cubic-beziers (Mode B
via `CB-EL-MOTION-1`); apple-dark = 150/300/500ms, ease-out/ease-out/ease-in ([derived],
proposed owner choice). Bounds law [70,700]ms stands. The three `*-motion-band` gap rows may
resolve to these values in slice 4 only after §16-H2 resolves.
...
### 2.2 Family labels (Mode D; own no axes; PROPOSED — pending §16-B)
...
### 2.3 The 30 proposed axis cells (lowerable only as their §16 rows resolve; DATA at `…/page_archetype/axes/<axis>`)

Every cell proposal: `value_id`, clause ids, exact value, negative case, mutant (§6.4). Mode-B
rows resolve through the clauses above; a Mode-D row enters `ratified.json` only after its §16
row resolves. Proposed AP/LG typography map (`R-W3-TYPE-1`, pending §16-H2): title 28px/700,
h2 20px/600, body 15px/400, sub 13px/500 (sizes from SHELL_SCALE history; weights are proposed
owner choices — Apple publishes no web literals).
...
- **Owner ratification via `R-W3-COMP-AP-1`/`R-W3-COMP-LG-1` (19, deterministic; PENDING
  §16-D, values §16-H2):** AP 11
  ...
  become owner-exact laws in slice 4 only once those rows resolve.
...
### 6.2 Contextual component DATA (slice 4; POLICY pending §16-D, exact values pending §16-H2)
...
`fact.control_labels`). These are the complete PROPOSED slice-4 values; they may not be lowered
or implemented as Mode-D authority until §16-H2 resolves.
...
THIS design at its binding review state (the committed rev-16 bytes; convergence closure — see
§15) and cross-checked equal to profile DATA — DATA and renderer omitting the
...
`TARGETS` → the §2.2 families once §16-B resolves; `REQUIRED_ANATOMY` → route-parameterized constants from
...
- Self-hosting: slice 1's own admission uses the current observation mechanism (declared once);
  slice 0's admission = the T0 doc-authority RED, already observed (§0). Its classification JSON
  stays IGNORED scratch evidence at `scratchpad/work/slice-0-admission.json` — referenced by the
  review record, never landing-commit cargo (`scratchpad/` is gitignored). Slice 1 lands the
  tracked envelope homes (`assets/receipts/admissions/`, `contracts/admission_ledger.json`);
  the slice-0 scratch evidence itself enters a committed governed home via the w3c-org intake
  lane (future work, sequenced after slice 0).
...
Scope (closed landing-patch allowed paths): `docs/plans/**`, `docs/history/**`, `AGENTS.md`,
...
`tests/contracts/test_doc_authority.py`, `requirements.txt`. `scratchpad/work/**` is DROPPED
from the roster: ignored scratch evidence is review-referenced, never commit cargo (this also
honors the w3c-org design's standing instruction). Slice 0 is TEXT-ONLY and lands via
plain-diff review — the staged closed-roster diff is the reviewed patch, sha256-pinned from
review to commit; the §13.2 hermetic patch machinery is slice-1+ CODE, not slice-0 cargo.
Admission:
...
the contracts root allowlist. Landing commit (branch `w3-correction`, FIRST commit — the
w3c-org lane (L1 org law, L2 intake) is unbuilt/unreviewed future work SEQUENCED AFTER this
landing, never a slice-0 precondition; made only AFTER the design family reaches its binding
review state under the effort law — here CONVERGED: rev15+addendum max REVISE and rev16 xhigh
REVISE, both folded — and CODE + ADVERSARIAL approve the exact landing patch): this design (the
committed rev-16 bytes at their binding convergence closure) + the handoff file + `AGENTS.md`
amendment (§1.1 incl. the H1 clause
...
- **`R-W3-RATCHET-1` (clean-context correction, r13; binds at slice 1 per the scoping above):**
  ...
  **Slice-0 SEED inventory (lawful byte-for-byte):** the seed additionally carries top-level
  `measurement_context` and `ratchet_status`; its ordinary inventory is seven rows using
  `fingerprint_mode: "seed-terminal-line"`; its loader inventory is nine bare-basename masks.
  These are seed counts, not fixed slice-1 output. Slice 1 runs the trusted pinned command and
  writes only the ordinary rows still observed: a zero-to-seven subset of the seed ids, with
  canonical atomic-member fingerprints. Resolved seed ids disappear; no new ordinary id is
  admissible. The canonical slice-1+ top-level key set is `{contract_id, schema_version, purpose,
  observed_utc, command, measurement_context, totals, setupclass_masks, loader_masks, failing}`;
  `ratchet_status` is removed. The authoritative seed remains 75 tests / 7 failures / 9 errors /
  16 unique observed ids. The seven possible ordinary ids are:
  ...
  `setupclass_masks` is empty in the slice-0 seed. Its exact loader-mask identities are the bare
  basenames `{test_dashboard_visual_authority, test_design_source_provenance,
  test_official_source_parity, test_page_archetype, test_reference_pack,
  test_rendered_fact_adversarial, test_rendered_fact_density_adversarial,
  test_rendered_fact_paint_adversarial, test_rendered_facts}`. The nine fully-qualified names in
  the list immediately below are canonical TARGET identities, not seed names; slice 1 retains only
  the still-unimportable zero-to-nine subset, each with exact keys
  `{module, fingerprint, unmask_rule}`:
  ...
  Slice 1 writes only the still-unimportable loader subset: zero to nine rows. Remaining masks use
  fully qualified module names and canonical fingerprints. A module imported in slice 1 loses its
  mask in that same slice and must satisfy the unmask rule; no stale mask survives. From slice 2
  onward the canonical typed identities, fingerprints, exact key sets, and frozen structure bind
  without a seed exception.
...
- **Parent-ledger subset algorithm (exact; r13 correction):** define a failure unit as either an
  ...
  Slice 1 is the declared seed→canonical transition and may also genuinely shrink: ordinary rows
  compare by seed test id, loader masks compare by seed basename, and the canonical result contains
  only the observed subsets described above. The trusted runner, not caller-supplied output or a
  fake extractor, derives the candidate. From slice 2 onward full canonical typed-unit identity and
  frozen structure bind exactly.

  Ratchet r5 is REJECTED review input, not evidence that this transition works. Its live run changed
  identities and failed the transition, and its new governed files were not homed. Before transition
  capability is claimed, the next ratchet slice must close all six r5 findings: live-transition
  closure; Git-environment provenance; trusted closed-schema regeneration; conformant mask rules;
  six-field packet binding; and stale shell documentation — plus every repo-layout homing failure.

  Slice 1 is the declared schema-normalization step: its parent is the slice-0 SEED, so (b) is
  asserted at test-id granularity for ordinary rows and at module-basename granularity for
  loader masks, and (d) is asserted against the canonical slice-1+ schema (the seed carve-out
  above governs the seed→slice-1 normalization). From slice 2 onward the full typed-unit
  identity and frozen structure bind exactly as written.
...
- **Patch-isolated review-BEFORE-commit sequence (r5 must-fix 4; restores the binding
  ...
  (1) Fable authors the approved design and REDs; (2) after admission, Opus builds only in an
  ...
  THERE and regenerate `contracts/correction_baseline.json` from that observation using the
  atomic-fingerprint and loader-mask rules above; any ledger carried from the dirty main worktree
  is discarded. GENERATE the slice's MF1 receipts THERE, stage EVERYTHING in the allowed paths,
  and emit the FINAL PATCH with the ONE canonical, HERMETIC, binary-safe command
  ...
  **The hermetic mechanism is CODE, not transcribed prose (r13c convergence-law correction —
  ...
  Never history rewrites; the dirty primary checkout is never stashed/reset and unrelated staged
  state remains staged (lossless). No step here authorizes a merge or push to branch `main`.
- **Transitional composer registry (debt timing fixed per r3):** slice 2 lands
  `scripts/rendering/composers/__init__.py::COMPOSERS` with each design-declared `renderer_key` →
...
7 Liquid family · 8 integrated evidence + divergence + full state matrix · 9 fixture/manual
reference-lane readiness (§8) + skill strengthening (§1.2 rows 16-18), with no live external
request or target write · 10 integrated review, full suite green (ratchet empty), clean reviewed
`w3-correction` checkpoint. Real crawl/APPLY is a later admitted lane; no `main` action.
...
6 → §13.2 ratchet. Correction acceptance: DEMO A (slices 5-7), DEMO B readiness (slice 9),
and the clean reviewed checkpoint (slice 10); real crawl/APPLY follows under fresh admission.
...
Round 14 (independent landing + design review, 2026-07-16): an independent Codex session
reviewed the exact rev-14 bytes
(sha256 `20dc35cabedfbda2c0632635c077e7bbe715ccb7e26b7617fc491ca8247ee8a5`) and the staged
15-path landing patch (sha256 `a530f3e58b0c58d77ea6a0f6f06a705e697576f15aea6618256121739bfbe2f4`):
`VERDICT: revise` + `ADVERSARIAL-VERDICT: revise` on the patch and `DESIGN-VERDICT: REVISE` on
the design — six substantive findings (missing independent approval on the exact bytes; §16 not
propagated; seed-schema/ratchet mismatch; scratch admission named as commit cargo; tracked-only
lifecycle scope understated; committed docs pointing outside the landed set). The round-13
"APPROVED" line above is SUPERSEDED as a verdict: that session authored the rev-13 replacement
blocks it then approved (author-never-approves), so rev 14 carried no binding independent
approval. The r14 REVISE is the first independent verdict on these bytes; it stands.

Rev 15 (2026-07-16): authored by FABLE (design author) while an emergency seat draft (Codex to
orchestrate/integrate/commit) was under consideration; that draft stayed scratch, was never
ratified, and the operator restored Fable-conducts in-session on 2026-07-16 — rev 16 carries
the restored routing (no artifact author approves itself). The Codex fix-author proposals
(`scratchpad/work/codex-slice0-fix-author-2026-07-16.md`) were review INPUT: each was
independently verified against the live bytes and re-authored, so rev 15 has a non-Codex author
and Codex may gate it. Rev 15 folds all six r14 findings and settles the landing order: slice 0
is the FIRST `w3-correction` commit; the w3c-org L1/L2 intake lane follows as its own gated
future lane. Rev 15 anticipated binding Codex verdicts on its exact bytes; no approving verdict
or carrying commit ever existed — its combined review returned REVISE (see below) and rev 16
supersedes it. This file does NOT restate its
own approval (writing a verdict into the reviewed artifact re-taints the bytes — the very
round-12/13 failure shape). The combined rev-15 fold plus direct operator branch addendum then
received its FIRST combined independent review at max effort on 2026-07-16 and returned
`DESIGN-VERDICT: REVISE` with six findings. That result supersedes rev15's pending state.

Rev 16 (this revision, 2026-07-16) is Fable-authored adoption of an isolated local fallback
fold, re-authored where the fallback carried the emergency Codex-integrates seat model: the
operator restored the Fable-conducts routing in-session (Fable verifies, integrates, and
commits; Codex reviews only). It closes all six max-review findings: operational seat routing;
post-checkpoint real crawl/APPLY; seed-subset canonicalization with rejected r5 input; coherent
L3/CR-5 versus L4/CR-11 ownership; exact 15-path cargo; and byte-clean ACTIVE replacements. It
issues no verdict. Its repeat independent DESIGN review ran at xhigh on 2026-07-16 and returned
REVISE with one finding (Fable-conducts propagation through surviving rev15 blocks), folded by
R16-15 under the convergence law — the second REVISE on this family, so no further prose DESIGN
round runs. The binding verdicts are the exact-patch CODE + ADVERSARIAL reviews; until they
approve, rev16 is UNAPPROVED author output.

## 16. Operator ratification block

| Row | Law | Proposal | Decision / identity / date |
|---|---|---|---|
| A | Actor build-lane amendment | §1.1 | CONFIRMED 2026-07-16 — the operator restated the law in-session in their own words: Fable designs + authors REDs; Codex reviews (the designs and the finished builds) and never codes; Opus builds; same SOP as semantic-tdd. Recorded as paraphrase of the operator's real words, on top of the original directive (digest `:753`); no invented quote. |
