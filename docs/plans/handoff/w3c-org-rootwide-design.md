...
# r3 DELTA — folded into this design of record (2026-07-17)

> Reviewed lifecycle transition (org L1): the 2026-07-17 r3 amendment record is folded in
> below verbatim (its own companion-artifact preamble excepted). The doc cap is unchanged;
> this doc occupies the org design slot. Its `doc_authority_policy.toml` lifecycle row is
> added in the same reviewed transition.

# Org root-wide design — r3 DELTA (amendment record, 2026-07-17)

Producing model: Fable 5 (claude-fable-5) — execution metadata only. Seat: DESIGN + RED author,
org lane (conductor dispatch 2026-07-17). This is an AMENDMENT to the design of record, never a
second design: it folds the 2026-07-17 bindings into
`docs/plans/handoff/w3c-org-rootwide-design.md` rev 2
(sha256 ebcbee916b558a12e6f66015e5eb8da636c37939d6015b22b973b8db4f867413, verified byte-exact
against `.claude/worktrees/org-build/docs/plans/handoff/w3c-org-rootwide-design.md`). The L1
patch folds this text into the tracked design doc in the same reviewed lifecycle transition.
All code anchors below were verified against the committed slice-0 tip `74fe3ecd`
(`git show 74fe3ecd:<path>`), never the checkpoint main tree.

Companion artifacts (this round):
- RED replacements: `scratchpad/active/org/red/test_org_rootwide.py`
  (sha256 3286939036083b6bdf2680a2fa0d66fecacefd2e87d4b71311acd69b723af810) and
  `scratchpad/active/org/red/test_process_docs_currency.py`
  (sha256 0a03786b3a65e4271af77af60150aa74b71d548e36fdae50ff7f302f596fe14d).
- Convergence record: `scratchpad/active/org/red/org-red-r3-convergence-2026-07-17.md`.
- Build packet delta: `scratchpad/active/org/cargo/packet-org-l1-build-r3-2026-07-17.md`.

## A1 — pre-commit hook rides org L1 (CR-18; guards-gap binding 1)

The universe-B `core.hooksPath` row stops being passive law. L1 lands a TRACKED hook directory
`scripts/organization/hooks/` whose `pre-commit` (index mode 100755) pins EXACTLY the two landed
guard modules `tests/contracts/test_doc_authority.py` + `tests/contracts/test_structural_layout.py`
— every commit, every branch — and never runs discovery (the failure ratchet intentionally holds
RED tests; a discovery hook would block every commit).

RESOLVED CONTRADICTION (recorded, executable): the binding calls these "the two landed green
tests," but observed at `74fe3ecd` (faithful `git archive` copy, index initialized),
`test_structural_layout` fails 5 of its arms — exactly the five identities held in
`contracts/correction_baseline.json` `failing` rows (contract rows for slice-1 modules the
slice-patch lane has banked but not landed). `test_doc_authority` is fully green (8/8). A
raw-pinned hook would therefore block every org-L1-era commit. The hook law is RATCHET-AWARE:
it fails only on failures OUTSIDE the baseline identities (subset law, same grammar as the
ratchet). CR-18 encodes this: hook text must name both modules, must not contain `discover`,
and must cite `correction_baseline`. The mechanism: the hook invokes a small tracked runner
(`scripts/organization/hooks/run_pinned_guards.py`) that runs both modules and compares failures
against the baseline identities. Local `core.hooksPath` must be unset or resolve into the
governed dir (clean clones stay green; activation on the working machine is one config line,
ratified via the AGENTS.md step-12 amendment riding the org/W8 doc window).
The `live_controls` git row carries this as data: `hooks_path_policy = {governed_dir, pinned_tests}`.

## A2 — worktree governance and the ORPHAN-WORKTREE RED (CR-17; consolidation law 2026-07-17)

New contract section `worktree_governance`:
- `live_lanes`: typed rows `{lane, path_suffix, expires_utc}` — at most ONE build room per
  ACTIVE lane plus the landing vehicle (ceiling law). Expired rows cover nothing (liveness).
- `retirement_receipts_root`: `scratchpad/active/org/retirement` — pending-retirement receipts
  name the exact registered path.
Law: any registered git worktree (from `git worktree list --porcelain`, primary excluded) with
neither an unexpired live-lane row nor a receipt naming it is a FAILING TEST. Deterministic in a
clean clone (no extra registrations there). Today 27 linked rooms are registered; the
2026-07-17 retirement audit receipts cover the RETIRE-PENDING batch; the keep rooms
(`scratchpad/wt/w3-landing`, `.claude/worktrees/w8o-build`, plus the one active build room per
lane) get live rows at L1. Transient witness/verify jobs use throwaway `git archive` copies under
the session scratchpad — never `git worktree add` (this round's RED observation followed that
rite: created, receipted in the convergence record, removed).

## A3 — closed scratch grammar, data-driven birth router, retirement checkpoint (CR-15/16/20)

Closes the visible flat-scratch defect, not merely gitignores it.
- `scratch_grammar` section: exactly two working roots — `scratchpad/active/<lane>/<kind>/...`
  (current regenerable inputs) and `scratchpad/archive/<YYYY-MM-DD>/<lane>/<kind>/...`
  (superseded local copies). `scratchpad/work` is FROZEN legacy (no new files ever). The section
  names the supersession-pointer artifact (A4).
- `birth_routing` section: a data table over `(producer, artifact_kind, proposed_path)` with a
  closed home vocabulary — `dev-reports:<one-of-the-closed-five>` (never a sixth category),
  `assets-receipts` (public acceptance receipts with provenance), `custody` (raw/frozen
  third-party material, descriptor-bound, never a repo path), `scratch-active`,
  `scratch-archive`. Exact-one home per (producer, kind); duplicate rules are a table defect.
  Refusals: traversal, absolute path, backslash, non-NFC, unknown kind/producer/lane, extension
  mismatch, direct file under `scratchpad/`, any new file under `scratchpad/work/`,
  producer/lane mismatch, and tracked-record kinds (plan, admission, gate transcript, receipt)
  choosing ANY scratch destination (ignored archives are recovery aids, never authoritative).
  A refused birth performs no mkdir and no write — the hostile RED proves byte-identical trees,
  including through a symlink component. The production router
  `scripts/organization/birth_router.py` (L1) conforms to the reference in the RED; the REAL W6
  producer calls it before its first mkdir/write, and the bypass is killed by a W6-lane
  integration test (bound in the W6 packet when it rebinds, not in org L1).
- `.gitignore` gains the root-anchored roster `/scratchpad/`, `/mutants.out/`, `/.hypothesis/`,
  `/reports/` (at `74fe3ecd` there is NO scratchpad ignore line at all; the checkpoint tree's
  line is unanchored — both defects close at L1). Anchoring behavior is proven hermetically by
  `git check-ignore --no-index` probes in the RED.
- `scratch_citation_dispositions` section (L1) + retirement checkpoint (flips L2/L3): every
  tracked file citing `scratchpad/` carries exactly one disposition row — `grammar-only`
  (.gitignore/AGENTS tree-name mentions, no scratch FILE path) or `dangling-history` (explicit
  record, e.g. `d-shell-2-design.md`'s missing `scratchpad/d-shell-0-design.md`: promote a
  verified source or record the disposition, never invent bytes). Promotion removes the literal
  (coupled path+digest replaces it), so promoted files simply leave the offender set. The final
  probe: `scratchpad/work/` empty or absent. Migration is LOSSLESS: durable evidence is copied
  and digest/back-reference verified in `dev/reports/<category>/` FIRST; only then are ignored
  source copies mechanically moved to the dated archive with local inventories; active files
  move to their typed lanes; NOTHING is deleted without operator ratification. The six tracked
  citers at `74fe3ecd`: AGENTS.md, docs/history/PLAN-LEDGER.md, docs/plans/ACTIVE.md,
  docs/plans/handoff/HANDOFF.md, docs/plans/handoff/d-shell-2-design.md,
  docs/plans/handoff/w3c-0-design-closure.md.

## A4 — scratch fork visibility (guards-gap binding 3)

The org-lane preflight (`scripts/organization/scratch_preflight.py`, L1, tracked; runs locally,
never a clone-blocking contract test) gains the fork-visibility arm: content-kin detection over
the scratch grammar. Kin = same basename after stripping a producer prefix
(fable-/local-/codex-/opus-), so the live C4 case (two divergent rev16 folds) is caught.
Identical digests across paths are LAWFUL multiplicity (the eight W6 duplicate-digest groups are
determinism witnesses — path multiplicity is preserved, or every original path+digest is
manifested before any dedup). Divergent bytes under one kin key redden unless a supersession
pointer names every loser (the rev16 fallback-fold precedent is the lawful disambiguator). L2
intake content-addresses lineage. The detector logic is contract-tested hermetically in the RED;
the live sweep stays a preflight finding.

## A5 — bootstrap-anchor consumer ledger (CR-19; checkpoint binding)

`contracts/repo_layout.json` is the ONLY loose bootstrap anchor; every presently unlanded
contract enters a semantic typed home directly (no additions to legacy-root debt). Its consumer
set is governed by a `bootstrap_anchor_consumers` section: canonical sorted rows
`(consumer_path, literal_occurrence_count, consumer_kind)` with a digest over the canonical
JSON, an owner, an expiry, and SUBSET-ONLY shrink against counts RECOMPUTED from the
candidate-index blobs (`git grep -c --cached`, anchor excluded as its own consumer). The prose
count `88` is NOT encoded anywhere; the 2026-07-16 audit's 48/25, 80/32, 82/33 measurements are
diagnostic proof that a magic count is unstable, nothing more.

## A6 — intake independence + evidence routing intents (finding 1; L2)

The L2 intake manifest (`dev/reports/admissions/w3c-org-intake.json`) no longer self-attests:
completeness derives from an INDEPENDENT whole-scratch cutoff roster landed as its own tracked
artifact (`dev/reports/admissions/w3c-org-intake-roster.tsv`, one row per source path —
multiplicity preserved), pointed to by `{path, sha256}` from the manifest and recomputed by the
guard. Bijection both ways; every source classifies exactly once as
durable-evidence | perishable | stray; every durable destination is under `dev/reports/`; every
row carries its source digest. Counts stay diagnostic (188 files / 30,258,374 bytes at the
2026-07-16 census; recomputed at the real L2 cutoff). Routing intents at intake:
final independent Codex verdicts -> `design_reviews`; admissions/ratifications/operator records
-> `admissions`; worker audits/dossiers -> `worker_reviews`; the 49-file W6 demo tree -> a
subject-scoped `archive` home; NO current scratch artifact qualifies for `role_assignments`.
`dev/reports/design_reviews/AUTHORIZED-INTENT-MATH-PROOF-1/` gets an explicit adopt/remove
disposition row, never inferred membership. The un-rostered applied rev16 fold
(sha 73cfcd6e…) gets a roster row at the real cutoff; the divergent `active/` twin is
disambiguated by a supersession pointer (A4).

## A7 — row-law strengthenings (findings 2–8; law lives in the RED files)

- Governed pointers are COUPLED `(path, digest)` pairs on one line; targets must exist, be
  tracked, and recompute. Uncoupled paths, deleted targets, and unrelated hashes all redden.
- Debt rows: never empty-baseline; exercised against the ACTUAL candidate index (present
  members, residue growth via the shape classifier, stale rows, expiry).
- Shape rows: node kinds from the closed git-mode vocabulary; typed pattern lists; the declared
  classifier RUNS over the real index — exact-one partition with only debt baselines absorbing
  legacy residue.
- Live controls: every row complete and typed (exact `.claude`/`.git`/`.venv` paths, typed cache
  list, boolean-False `authoritative`, boolean-True `mirror_parity_check`, exact registry
  authority, hooksPath policy on the git row).
- Manifest rows: typed schema_version (positive int, not bool), ISO-8601 UTC timestamps, typed
  retention (`keep-newest-N` numeric), non-empty identities, tracked existing artifacts, digest
  recompute, REAL back-references (citing doc exists, tracked, and cites path or digest);
  finalized rows require digest-bearing citations and are exercised against real manifest state;
  permanent-retention rows land finalized.
- Concept rows: every required id bound to its CANONICAL kind
  (sop_process/demo/mode_ladder/role); sop owners are `AGENTS.md#<their-own-block-id>` exactly;
  role owners match the registry EXACTLY (no substrings); demo/mode-ladder rows never project
  from AGENTS.
- SOP markers bind the canonical NAMED section (keyword-witnessed heading/body); a marker before
  unrelated prose ("Cooking") reddens; pointers resolving to empty or wrong sections dangle.
- AGENTS scratch law precision: the step-13 scratch review-queue routing and any CONCRETE
  `scratchpad/work/<file>` citation redden; the bare tree name may survive only as
  grammar teaching under a `grammar-only` disposition (A3).

## A8 — W7/org bootstrap cycle + document cap (sequencing law, restated as binding)

After slice 0: W7 lands a NON-ENFORCING registry/code bootstrap first (creates no new
`dev/reports` category paths; queues its review evidence for org intake; enforcement disabled).
Org L1 then lands the root-wide shape law + the closed five homes
{design_reviews, admissions, worker_reviews, archive, role_assignments} with empty typed
manifests and closes the `.claude` live-control row against the now-tracked W7 registry. W7
ACTIVATION lands only after L1; the legacy W7 names `dev/reports/reviews` and
`dev/reports/audits` are invalid — assignments route to `role_assignments`, design/code gate
records to `design_reviews`, worker/runtime audits to `worker_reviews`. Org L2/L3 then
W7-dependent L4 follow; never a sixth category. Document cap: three live handoff rows at start;
W7 may occupy slot four and the org design slot five; W7's handoff design folds/supersedes in
the same reviewed transition when activation closes, before another live design enters; the org
design folds when L4 completes; W6 and W8 enter only in their later lanes;
`max_live_handoff_docs` is never raised to admit concurrent prose.

## A9 — W6 contract-id destinations (before the W6 packet rebinds)

Typed destinations, all inside existing vocabularies (no new roots): completeness feature/tag
contracts, vocabulary/capability projections, identifier allowlist + lock, lifecycle graph, and
demo-target contracts -> semantic typed homes under `contracts/` (root_allowlist or a declared
group dir in the same slice that creates them — never loose additions to legacy-root debt);
custody stores for raw third-party material -> `custody` descriptors via the birth router (A3),
never repo paths; current reference/design/official-source contracts stay in their existing
`contracts/{design_profiles,official_sources,reference_packs}` group dirs; slice-0
doc-authority/baseline contracts remain `contracts/doc_authority_policy.toml` +
`contracts/correction_baseline.json` (already landed). The W6 demo evidence tree ->
`dev/reports/archive/w6-demo-2026-07-16/…` at L2 (A6).

## Landing chain with the deltas folded (unchanged order, new obligations attached)

- L1: contract sections (authority transition, projections, dev_reports, live_controls +
  hooksPath policy, shape rows, debt ledger, birth_routing, scratch_grammar,
  worktree_governance, bootstrap_anchor_consumers, scratch_citation_dispositions) + empty
  category homes + anchored .gitignore + hook script/runner + birth router + preflight + family
  READMEs + the two RED files landed as tracked tests (rosters updated) + ratchet baselining of
  the still-red-by-design arms. Flips: CR-1..4, 6..9, 14..19 (+ CR-20's section arm).
- L2: independent cutoff roster + intake manifest + content-addressed copies + dispositions
  executed. Flips: intake RED; CR-20's citation/work-empty probes move to flippable.
- L3: slice-0 regeneration with coupled governed pointers. Flips: CR-5, CR-20 fully.
- L4 (after W7): docs-currency edits (SOP blocks, pointers, policy TOML, AGENTS routing).
  Flips: CR-10..13.
