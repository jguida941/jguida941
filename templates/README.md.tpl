...
| Metric | Value |
|--------|-------|
| Commit Contribs (Display Metric) | `{{ snapshot.total_commits }}` |
| Public Repo Commits (Owned Non-Fork) | `{{ snapshot.public_scope_commits }}` |
| All-Time Commit Contributions (GraphQL) | `{{ snapshot.all_time_commit_contributions if snapshot.all_time_commit_contributions is not none else "n/a" }}` |
...
**Commit metric scope:** `{{ data_scope.commit_metric_scope }}`
...
| [**{{ c.repo }}**]({{ c.url }}) | `{{ c.activity }}` | {{ c.time_ago }} |
{%- else %}
